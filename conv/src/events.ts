...
export function createAuthorizedEvent(
  ...
  return ctx.runMutation(internal.events.createForAuthorizedSession, event);
}
...
export function updateAuthorizedEvent(
  ctx: GenericActionCtx<DataModel>,
  id: Id<"events">,
  event: EventType,
) {
  return ctx.runMutation(internal.events.updateForAuthorizedSession, {
    id,
    event,
  });
}

export function listAuthorizedEvents(
  ctx: GenericActionCtx<DataModel>,
  sessionId: Id<"sessions">,
) {
  return ctx.runQuery(internal.events.listForAuthorizedSession, { sessionId });
}
...
export const createForAuthorizedSession = internalMutation({
  handler: async (ctx, args) => {
    const event = eventSchema.parse(args);
    await ensureSessionWritable(ctx, event.sessionId);
    return ctx.db.insert("events", event);
...
export const listForAuthorizedSession = internalQuery({
  args: {
    sessionId: v.id("sessions"),
  },
  handler: async (ctx, args) => {
